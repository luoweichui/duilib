...
namespace DuiLib {
	...
	{	
		...
		if(_tcsicmp(msg.sType, _T("listheaditemchecked")) == 0)
		{
			...
			int  nIndex = static_cast<int>(msg.wParam);
			//判断是否是本LIST发送的notify
		...
		else if (_tcsicmp(msg.sType, DUI_MSGTYPE_LISTITEMCHECKED) == 0)
		{
			for (int i = 0; i < GetCount(); ++i) {
				...
				{
					OnListItemChecked(LOWORD(msg.wParam), HIWORD(msg.wParam), static_cast<BOOL>(msg.lParam));
					break;
				}
			}
		}
		...
		{
			int  iCurSel, iOldSel;
			iCurSel = static_cast<int>(msg.wParam);
			iOldSel = static_cast<int>(msg.lParam);
