...
namespace DuiLib
{
	...
	void CTrayIcon::TrayIconSetFocus()
	{
		Shell_NotifyIcon(NIM_SETFOCUS, &m_trayData);
	}
	...
	void CTrayIcon::SetHide()
	{
		NOTIFYICONDATA nid;
		nid.cbSize = sizeof(NOTIFYICONDATA);
		nid.hWnd = (HWND)m_trayData.hWnd;
		nid.uID = m_trayData.uID;
	...
	bool CTrayIcon::SetNewIcon(NOTIFYICONDATA& trayData)
	{
		memcpy(&m_trayData, &trayData, sizeof(m_trayData));
		Shell_NotifyIcon(NIM_MODIFY, &m_trayData);
		return true;
	}

	bool CTrayIcon::GetCurrentData(NOTIFYICONDATA* ptrayData)
	{
		if (ptrayData!=NULL)
		{
			memcpy(ptrayData, &m_trayData, sizeof(m_trayData));
			return true;
		}
		return false;
	}
